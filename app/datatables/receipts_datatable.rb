class ReceiptsDatatable < GenericDatatable
  ...
  def records
    ...
    if params[:search][:value].present?
      receipts = receipts.where("members.name ilike :search or receipts.kind ilike :search", search: "%#{params[:search][:value]}%")
                   .references(:member)
    end
    receipts
  end
  ...
  def columns
    %w(receipts.id receipts.received_date receipts.member receipts.kind receipts.mode receipts.amount)
  end
end
